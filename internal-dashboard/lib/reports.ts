...
export async function buildWeeklyReportBody() {
  ...
  rows.forEach((row, index) => {
    lines.push(
      ...
      "",
      "Evidence URL:",
      row.evidenceUrl || "Review required",
      "",
      "Evidence summary:",
      row.evidenceSummary || row.trigger || "Review required",
      "",
      "Why it matters for freight:",
      row.whyItMatters || "The operating change may create a timing window for capacity, routing, warehousing, or backup-provider review.",
      "",
      "Likely freight need:",
      row.likelyNeed || row.relevance || "Review required",
      "",
      "Best-fit provider type:",
      row.bestFitProvider || row.likelyNeed || row.relevance || "Review required",
      "",
      "Buyer/contact path:",
      ...
      "",
      "Next action:",
      row.nextAction || "Open the evidence URL, verify the operating change, then route outreach through the listed buyer path.",
      ""
    );
  });
